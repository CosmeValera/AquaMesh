...
- Hosted AI Study Guides do NOT run the client `provider.ts` orchestration. They take a separate server-side path: `generateMonolithHostedStudyGuide` in `api/hosted-ai.ts` builds the whole guide (title, pages, Quick Start, and the known-topic `contextPlan`) in one `buildMonolithGuidePrompt` call, then a second call for the final quiz. When changing Study Guide prompt behaviour, change both that monolith prompt and the BYO prompts in `apps/studymesh/src/studyGuides/quickStart.ts`, or hosted users will not see the change.
- Known-topic bridges ("explain this through what I already know") never let the model grade its own bridge. The model lists `targetParts` and `correspondences`; `deriveStudyGuideBridgeStrength` in `quickStart.ts` counts them and decides none/weak/strong. Both paths share that function. Asking the model directly whether a bridge was strong made it answer "weak" for every candidate, including same-field ones. Do not reintroduce a model-supplied `bridgeStrength`, a same-domain preference in the selector, or an unconditional "say where the comparison breaks" instruction.
- Reasoning effort is per stage in `api/hosted-ai.ts` (`REASONING_OPENAI_STAGES`). Only the bridge-mapping stages get a budget; everything else stays at `none`.
- `apps/studymesh/tests/live/` holds live-network harnesses for this flow. They cost real credits, are excluded from `test:unit`, and are run by hand with `npx tsx`. **Read `apps/studymesh/tests/live/README.md` before generating Study Guides from a script**: it covers how to call the hosted model directly, the collect-once-then-sweep-offline workflow that keeps threshold tuning nearly free, cost per guide, and the filters already measured and rejected. Agree a call budget with the user first.
...
- Dashboard chat source planning: `apps/studymesh/src/dashboardChat/sourcePlanner.ts` plans answer sources (Study Guide, general knowledge, web) before `askDashboard.ts` runs. It owns the Auto-mode policy overrides, web search-query rewriting, exact-list count detection (`exactAnswerCount`), and the no-LLM fallback plan. The planner LLM call is skipped for explicit non-web source selections and smalltalk never reaches it.
