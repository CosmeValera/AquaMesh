...
describe('KnownTopicsPanel', () => {
  ...
  it('blocks adding more topics once the storage limit is reached', async () => {
    storage[PROFILE_CONTEXT_STORAGE_KEY] = JSON.stringify({
      version: 1,
      roles: [],
      broadKnowledge: [],
    ...
    expect(
      JSON.parse(storage[PROFILE_CONTEXT_STORAGE_KEY]).specificKnowledge,
    ).toHaveLength(USER_KNOWN_TOPICS_STORAGE_MAX)
  })
