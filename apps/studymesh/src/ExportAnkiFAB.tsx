...
const ExportAnkiFAB: React.FC = () => {
  const [isOpen, setIsOpen] = useState(false)

  return (
    <>
      <Box sx={{ position: 'fixed', bottom: 24, right: 24, zIndex: 9997 }}>
        <Tooltip title="📤 Export to Anki" placement="left">
          <Fab color="primary" aria-label="Export Anki" onClick={() => setIsOpen(true)} sx={{ width: 56, height: 56, fontSize: '1.5rem', bgcolor: 'primary.main', '&:hover': { bgcolor: 'primary.dark' } }}>
            📤
          </Fab>
        </Tooltip>
      </Box>
      {isOpen && (
        <Box sx={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 9998, pointerEvents: 'none' }}>
          ...
          <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', pointerEvents: 'auto' }}>
            <ExportAnkiPanel onClose={() => setIsOpen(false)} />
          </Box>
        </Box>
      )}
    </>
