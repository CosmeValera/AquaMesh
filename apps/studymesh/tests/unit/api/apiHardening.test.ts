...
import { hashClientIp } from '../../../../../api/client-ip'
...
const GUEST_MONOLITH_GUIDE = {
  title: 'Bottlenecks',
  folderName: 'Bottlenecks',
  emoji: 'B',
  quickStart: {
    keyIdea:
      'A bottleneck is the single step that caps how fast the whole system can finish its work.',
    quickSummary:
      'Work moves only as fast as its slowest step.\n\nFinding that step first is what makes every other change worth making.',
  },
  pages: ['01 - Spotting it', '02 - Measuring it', '03 - Moving it'].map(
    (title, index) => ({
      title,
      summary: `${title} preview.`,
      rawNotes: `${title}\n\nComplete shipped lesson content for page ${
        index + 1
      }. Every paragraph finishes cleanly.`,
    }),
  ),
}

const GUEST_QUIZ_QUESTIONS = Array.from({ length: 6 }, (_, index) => ({
  question: `Guest question ${index + 1}?`,
...
const stubHostedAiEnv = () => {
  ...
  vi.stubEnv('GUEST_IP_HASH_SECRET', 'guest-ip-secret')
}

interface HostedAiRpcOverride {
  ok: boolean
  status: number
  payload: unknown
}

interface HostedAiCall {
  url: string
  body: Record<string, unknown> | null
}

/**
 * One hosted AI gateway happy path with every Supabase RPC and provider call
 * mocked, recorded in call order so guest tests can assert what ran and when.
 */
const createHostedAiFetchMock = ({
  isAnonymous = true,
  allowanceUsed = 1,
  rpcOverrides = {},
}: {
  ...
  rpcOverrides?: Record<string, HostedAiRpcOverride>
} = {}) => {
  const calls: HostedAiCall[] = []
  ...
  const fetchMock = vi.fn((url: string, init?: RequestInit) => {
    ...
    if (target.includes('/auth/v1/user')) {
      return Promise.resolve(
        jsonResponse(
          isAnonymous ? { id: 'guest-1', is_anonymous: true } : { id: 'user-1' },
        ),
      )
    }

    const overridden = Object.keys(rpcOverrides).find((rpcName) =>
      target.includes(`/rest/v1/rpc/${rpcName}`),
    )

    if (overridden) {
      const override = rpcOverrides[overridden]
      return Promise.resolve(
        jsonResponse(override.payload, override.ok, override.status),
      )
    }

    if (target.includes('/rest/v1/rpc/guest_get_allowance')) {
      return Promise.resolve(
        jsonResponse([
          {
            study_guides_allowed: 3,
            study_guides_used: allowanceUsed,
            upgraded_at: null,
          },
        ]),
      )
    ...
    if (target.includes('/rest/v1/rpc/guest_ip_reserve_study_guide')) {
      return Promise.resolve(jsonResponse(1))
    }

    if (target.includes('/rest/v1/rpc/hosted_ai_begin_usage')) {
      return Promise.resolve(jsonResponse({ status: { studyCredits: 0 } }))
    }
    ...
    if (target.includes('api.cerebras.ai')) {
      ...
      if (prompt.includes('Write a complete, final RabbitHole Study Guide')) {
        return Promise.resolve(
          jsonResponse({
            choices: [
              { message: { content: JSON.stringify(GUEST_MONOLITH_GUIDE) } },
            ],
          }),
        )
      }
        ...
        return Promise.resolve(
          jsonResponse({
            choices: [
              {
                message: {
                  content: JSON.stringify({ questions: GUEST_QUIZ_QUESTIONS }),
                },
              },
            ],
          }),
...
const hasCall = (calls: HostedAiCall[], needle: string): boolean =>
  calls.some((call) => call.url.includes(needle))

const callIndex = (calls: HostedAiCall[], needle: string): number =>
  calls.findIndex((call) => call.url.includes(needle))

const guestGuideRequest = (
  body: Record<string, unknown>,
  headers: Record<string, string> = {},
) => ({
  method: 'POST',
  headers: { authorization: 'Bearer guest-token', ...headers },
  body,
})
...
describe('API payment and hosted AI hardening', () => {
  ...
  it('rejects guest podcast generation before any provider call', async () => {
    stubHostedAiEnv()
    const { fetchMock, calls } = createHostedAiFetchMock()
    vi.stubGlobal('fetch', fetchMock)
    const { response, res } = makeResponse()
    ...
    expect(hasCall(calls, 'api.cerebras.ai')).toBe(false)
    expect(hasCall(calls, 'api.v8.unrealspeech.com')).toBe(false)
    expect(hasCall(calls, '/rest/v1/rpc/hosted_ai_begin_usage')).toBe(false)
  })

  it('rejects guest surfaces and raw schema generation outside Quick Guides', async () => {
    stubHostedAiEnv()
    const { fetchMock, calls } = createHostedAiFetchMock()
    vi.stubGlobal('fetch', fetchMock)
    const quickCreate = makeResponse()

    await hostedAiHandler(
      guestGuideRequest({
        action: 'generateWithQuickStart',
        surface: 'quick-create',
        parts: [{ text: 'Make flashcards from this dashboard.' }],
      }),
      quickCreate.res,
    )

    expect(quickCreate.response.statusCode).toBe(403)
    expect(quickCreate.response.body).toMatchObject({
      ok: false,
      error: { code: 'invalid_request' },
    })

    // Plain `generate` hands the caller's responseSchema to the provider, so it
    // would be a free structured-output oracle even on the Study Guide surface.
    const rawGenerate = makeResponse()

    await hostedAiHandler(
      guestGuideRequest({
        action: 'generate',
        surface: 'study-guide',
        parts: [{ text: 'Return this document as JSON.' }],
        responseSchema: {
          type: 'object',
    ...
    expect(rawGenerate.response.statusCode).toBe(403)
    expect(hasCall(calls, 'api.cerebras.ai')).toBe(false)
    expect(hasCall(calls, '/rest/v1/rpc/hosted_ai_begin_usage')).toBe(false)
  })

  it('rejects oversized guest prompts before any provider call', async () => {
    stubHostedAiEnv()
    const { fetchMock, calls } = createHostedAiFetchMock()
    vi.stubGlobal('fetch', fetchMock)
    const { response, res } = makeResponse()
    ...
    await hostedAiHandler(
      guestGuideRequest({
        action: 'generateWithQuickStart',
        surface: 'study-guide',
        // Split across parts: the cap is on the summed prompt, not one part.
        parts: [{ text: 'a'.repeat(3_000) }, { text: 'b'.repeat(1_500) }],
      }),
      res,
    )
  ...
  it('reserves the guest network budget from the first forwarded hop before charging usage', async () => {
    stubHostedAiEnv()
    const { fetchMock, calls } = createHostedAiFetchMock()
    vi.stubGlobal('fetch', fetchMock)
    const { response, res } = makeResponse()
      ...
      guestGuideRequest(
        {
          action: 'generateWithQuickStart',
          surface: 'study-guide',
          parts: [{ text: 'Explain bottlenecks' }],
        },
    ...
    expect(response.statusCode).toBe(200)
    const allowanceIndex = callIndex(calls, '/rest/v1/rpc/guest_get_allowance')
    const reserveIndex = callIndex(
      calls,
      '/rest/v1/rpc/guest_ip_reserve_study_guide',
    )
    const beginIndex = callIndex(calls, '/rest/v1/rpc/hosted_ai_begin_usage')

    expect(allowanceIndex).toBeGreaterThanOrEqual(0)
    // Read the allowance first so an exhausted guest never burns IP budget.
    expect(reserveIndex).toBeGreaterThan(allowanceIndex)
    expect(beginIndex).toBeGreaterThan(reserveIndex)
    expect(calls[reserveIndex].body).toMatchObject({
      p_ip_hash: hashClientIp('1.2.3.4'),
      p_owner_id: 'guest-1',
      p_daily_guide_limit: 12,
      p_daily_guest_limit: 5,
      p_global_daily_guide_limit: 300,
    })
    expect(calls[reserveIndex].body?.p_ip_hash).not.toBe(
      hashClientIp('5.6.7.8'),
    )
    expect(calls[beginIndex].body).toMatchObject({
      p_owner_id: 'guest-1',
      p_surface: 'study-guide',
    })
  })

  it('never reserves guest network budget for a signed in account', async () => {
    stubHostedAiEnv()
    const { fetchMock, calls } = createHostedAiFetchMock({
      isAnonymous: false,
    })
    ...
    expect(response.statusCode).toBe(200)
    expect(hasCall(calls, '/rest/v1/rpc/guest_ip_reserve_study_guide')).toBe(
      false,
    )
    expect(hasCall(calls, '/rest/v1/rpc/guest_get_allowance')).toBe(false)
  })

  it('maps the guest network ceiling to a rate limit without naming it', async () => {
    stubHostedAiEnv()
    const { fetchMock, calls } = createHostedAiFetchMock({
      rpcOverrides: {
        guest_ip_reserve_study_guide: {
          ok: false,
          status: 400,
          payload: {
            message:
              'Too many free Quick Guides from this network today. Create a free account to keep going.',
          },
        },
      },
    })
    ...
    expect(hasCall(calls, 'api.cerebras.ai')).toBe(false)
    expect(hasCall(calls, '/rest/v1/rpc/hosted_ai_begin_usage')).toBe(false)
    expect(hasCall(calls, '/rest/v1/rpc/hosted_ai_finish_usage')).toBe(false)
  })

  it('fails an exhausted guest allowance before the network budget is spent', async () => {
    stubHostedAiEnv()
    const { fetchMock, calls } = createHostedAiFetchMock({ allowanceUsed: 3 })
    vi.stubGlobal('fetch', fetchMock)
    const { response, res } = makeResponse()
  ...
  it('maps the guest allowance raise to guest_limit_reached, not insufficient credits', async () => {
    stubHostedAiEnv()
    const { fetchMock, calls } = createHostedAiFetchMock({
      rpcOverrides: {
        hosted_ai_begin_usage: {
          ...
          payload: {
            message:
              'Guest Quick Guide limit reached. Create a free account to keep your guides.',
          },
        },
      },
    })
    ...
    expect(response.statusCode).toBe(403)
    // The guest copy mentions Carrots, so the credits branch would swallow it
    // into a 402 and open the Carrot pack dialog instead of the sign-up panel.
  ...
  it('clamps the guest model timeout without clamping a signed in account', async () => {
    stubHostedAiEnv()
    ...
    const guest = createHostedAiFetchMock()
    vi.stubGlobal('fetch', guest.fetchMock)
    const guestResult = makeResponse()
    ...
    expect(guestResult.response.statusCode).toBe(200)
    expect(timeoutDelays).toContain(60_000)
    expect(timeoutDelays.some((delay) => delay > 60_000)).toBe(false)

    timeoutDelays.length = 0
    const account = createHostedAiFetchMock({ isAnonymous: false })
    vi.stubGlobal('fetch', account.fetchMock)
    const accountResult = makeResponse()
    ...
    expect(accountResult.response.statusCode).toBe(200)
